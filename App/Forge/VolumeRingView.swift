  ...
  var body: some View {
    ZStack {
      Canvas { context, canvas in
        ...
        let color: Color = sets > Double(mrv) ? Theme.negative : (sets >= Double(mev) ? Theme.accentValue : Theme.ramp[2])
        context.stroke(Path(ellipseIn: bounds), with: .color(color.opacity(0.18)), lineWidth: 6)
        if fraction > 0 {
