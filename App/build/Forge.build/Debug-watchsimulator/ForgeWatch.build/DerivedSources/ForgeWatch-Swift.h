...
#ifndef FORGEWATCH_SWIFT_H
#define FORGEWATCH_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="ForgeWatch",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
@class WCSession;
@class NSString;
SWIFT_CLASS("_TtC10ForgeWatch10WatchStore")
@interface WatchStore : NSObject <WCSessionDelegate>
...
@class HKSampleType;
@interface WatchStore (SWIFT_EXTENSION(ForgeWatch)) <HKLiveWorkoutBuilderDelegate>
...
@class NSDate;
@interface WatchStore (SWIFT_EXTENSION(ForgeWatch)) <HKWorkoutSessionDelegate>
- (void)workoutSession:(HKWorkoutSession * _Nonnull)workoutSession didChangeToState:(HKWorkoutSessionState)state fromState:(HKWorkoutSessionState)oldState date:(NSDate * _Nonnull)date;
