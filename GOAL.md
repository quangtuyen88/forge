...
- [x] Exercise art — 314 écorché illustrations, one per exercise, generated with the GPT image model through Codex from one style anchor (`ex-back_squat`). Each image's blue muscles, equipment and pose were verified against `ExerciseDB`; the audit found 82 of the 192 old grok images wrong and 122 exercises without art. Style and rubric: `docs/design/exercise-art-style.md`; generator `scripts/exercise-art-batches.py`; 600 px in `Assets.xcassets/ExerciseArt` via `scripts/import-exercise-art.py`; `ExerciseArt` still falls back to `MuscleThumb` when an asset is missing
