...
- [x] Logger — stats bar pinned under the navigation bar (Elapsed teal, Load blue, Sets green); active set card with thumbnail, segmented set bar, blue weight and green reps tiles, orange RPE row; exercise rows with a teal rest time and a green done/planned ring
- [x] Today/logger approval slice (23 Sep 2026; JEFIT primary reference, Home Workout secondary) — one main action per Today state (Resume, Check in, Train anyway, Start); plan rest day reads "Next up · <date>" with a secondary Start; an empty week offers only "Plan this week"; adjustment summary under the task card. Logger: receipt "Set n · saved on this device", "Target RPE" vs "Your RPE", Log set in the keyboard toolbar, double-tap and 0.6 s rest-tap guards, failed saves keep the numbers and start no rest, rest preview of the next set and target. A rest does not carry over after the app is killed; its leftover notification is cancelled when the logger opens.
...
- [x] Exercise art — 314 écorché illustrations, one per exercise, generated with the GPT image model through Codex from one style anchor (`ex-back_squat`). Each image's blue muscles, equipment and pose were verified against `ExerciseDB`; the audit found 82 of the 192 old grok images wrong and 122 exercises without art. A data audit then fixed six ExerciseDB entries (upright rows add the upper traps, the suitcase carry counts as core, the DB squeeze press is compound with triceps and front delts). Style and rubric: `docs/design/exercise-art-style.md`; generator `scripts/exercise-art-batches.py`; 600 px in `Assets.xcassets/ExerciseArt` via `scripts/import-exercise-art.py`; `ExerciseArt` still falls back to `MuscleThumb` when an asset is missing
