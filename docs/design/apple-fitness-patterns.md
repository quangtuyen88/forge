...
Colour rule: Forge does not adopt per-metric hues. Values default to `Theme.text`. The volume family (sets, tonnage, e1RM) uses `Theme.accentValue` (cobalt in light, `#6E93F0` in dark so numerals clear 6:1 on cards). Records, upward trends, readiness on a proceed day and completed rings use `Theme.positive`, mint `#34D399` on dark so the one number that matters reaches 9:1 on cards; downward trends, fatigue and heart rate use `Theme.negative`. Time is `Theme.text`. Units and captions are `Theme.textSecondary`. One accent-coloured value per row or cell, never two.
